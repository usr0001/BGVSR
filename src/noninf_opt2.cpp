...
float compute_ELBO_svi(int j, MatrixXdr &mu_k, MatrixXdr &alpha, MatrixXdr &sigma_k, double sigma_q1, double sigma_q2, double sigma_e,double pi_q1, double pi_q2, MatrixXdr &residual,int miniB, double varBeta)
{

        float ll = -g.Nindv/2* log(sigma_e);
	ll -= residual.cwiseProduct(residual).sum()/2/sigma_e; 
	ll -= g.Nindv *varBeta/miniB/2/sigma_e; 
...
void svi_step( MatrixXdr &residual ,MatrixXdr &mu_k, MatrixXdr &alpha, MatrixXdr &sigma_k, double &sigma_q1, double &sigma_q2, double &sigma_e, double &pi_q1, double &pi_q2, int miniB, int &t, float &ll, int step_a, int step_b, int iter)
{
		 ...
		 for( int j=0; j<g.Nindv/miniB; j++)
			...
			double update_f1 = sigma_e / ((g.Nindv-1)+1/sigma_q1); 
			double update_f2 = sigma_e /((g.Nindv-1)+1/sigma_q2);
			...
			{
				if(FunCatogory(i,0)==0)
					sigma_k(i,0)=update_f1; 
				else sigma_k(i,0) = update_f2; 
			}
			...
			for( int i=0; i<g.Nsnp; i++)
				...
				//replace XsXs(i,i) with 1 
				//since standardized genotype
		//		double sigma_k_update=sigma_e/((g.Nindv-1) + 1/sigma_beta);
		//		sigma_k(i,0) = sigma_k_update;		
			//	cout<<XsXs(i,i)<<endl; 
				
				//update mu
				MatrixXdr snp_cur = geno_matrix.block(i,0, 1, g.Nindv);
				MatrixXdr Xresidual = snp_cur * residual;
				residual = residual + snp_cur.transpose()*mu_k(i,0)*alpha(i,0); 
				double mu_k_update1 = Xresidual(0,0)  + (g.Nindv-1) *mu_k(i,0)* alpha(i,0);
                                mu_k_update1  = mu_k_update1 * sigma_k(i,0) / sigma_e ;	
				mu_k(i,0) = mu_k_update1;
				//update alpha
			...
			result  = result.cwiseProduct(alpha);
			 ...
			 MatrixXdr mua_mua = weight_mu.transpose()*weight_mu;	
			cout<<"MUAMUA: "<<mua_mua(0,0)<< " result sum: "<<result.sum()<<endl; 
			 double varBeta = (result.sum()- mua_mua(0,0))*(g.Nindv -1);
			 ll=compute_ELBO_svi(j, mu_k, alpha, sigma_k, sigma_q1, sigma_q2, sigma_e, pi_q1, pi_q2, residual, miniB,varBeta);
			 cout<<iter<<"\t"<<t<< "\t"<<ll;
			 double vg = pi_q1 * (g.Nsnp-FunCatogory.sum()) * sigma_q1 * sigma_e + pi_q2 *  sigma_q2 * sigma_e*(FunCatogory.sum());
			double h2g = vg / (vg + sigma_e);
			 cout<<"\t" <<pi_q1 <<"\t" << pi_q2 <<"\t"<<sigma_q1*sigma_e*pi_q1*(g.Nsnp- FunCatogory.sum())/(vg+sigma_e) << "\t"<< sigma_q2*sigma_e*pi_q2*(FunCatogory.sum())/(vg+sigma_e)  << "\t" << vg <<"\t" <<h2g<<endl;
			t++; 
			if(!first_update)
				ll_det = ll-prevll;
			 first_update = false;                         
		...
		//sigma_q1, q2, pi_q1, q2, sigma_e	
			...
			//residual = pheno - geno_matrix.transpose()*weight_mu;
			MatrixXdr temp = residual.cwiseProduct(residual);  
			
			double sigma_e_update1 =temp.sum() + varBeta;


			//debug
			//MatrixXdr ysys = ys.transpose()*ys; 
		//	double sigma_e_update2 = ysys(0,0); 
		//	MatrixXdr Xsys = Xs * ys;
		//	MatrixXdr yX_mu_alpha= Xsys.cwiseProduct(weight_mu); 
		//	sigma_e_update2 = sigma_e_update2 - yX_mu_alpha.sum()*2; 
		//	sigma_e_update2 += (g.Nindv-1)*result.sum(); 
			for(int b=0; b<miniB; b++)
			{
				MatrixXdr ind_i = geno_matrix.block(0,b,g.Nsnp, 1); 
				ind_i = ind_i.cwiseProduct(mu_k); 
				ind_i = ind_i.cwiseProduct(alpha); 
				MatrixXdr ind_i_2 = ind_i.cwiseProduct(ind_i); 
				double temp_sum = (ind_i.sum() * ind_i.sum() ) - ind_i_2.sum(); 
				sigma_e_update1 += temp_sum; 
		//		sigma_e_update2 += temp_sum; 
			}
	
			//debug
			double sigma_e_update = sigma_e_update1; 
			...
			cout<<iter<<"\t"<<t<< "\t"<<ll;
                	double vg = pi_q1 * (g.Nsnp-FunCatogory.sum()) * sigma_q1 * sigma_e + pi_q2 *  sigma_q2 * sigma_e*(FunCatogory.sum());
                	double h2g = vg / (vg + sigma_e);
			//cout<<"Total estimated variance: "<< vg+sigma_e; 
			//cout<<"   pheno variance: "<< (pheno.transpose() * pheno).sum() <<endl; 
               	 	cout<<"\t" <<pi_q1 <<"\t" << pi_q2 <<"\t"<<sigma_q1*sigma_e*pi_q1*(g.Nsnp- FunCatogory.sum())/(vg+sigma_e) << "\t"<< sigma_q2*sigma_e*pi_q2*(FunCatogory.sum())/(vg+sigma_e)  << "\t" << vg <<"\t" <<h2g << endl;
			t++; 
		}	
		return; 		
...
//	cout<<"printing std: "<<endl<<stds<<endl; 	
	...
	//normalize phenotype
	MatrixXdr pheno_sum = pheno.transpose() * pheno; 
	double pheno_variance = sqrt(pheno_sum(0,0)); 
	pheno = pheno/ pheno_variance; 
...
//	cout<<"Initializing prarameters..."<<endl<<"a: "<<a <<endl<<"l: "<<l <<endl; 
	...
	//sigma_e =0.2; 
	//sigma_q1 = sigma_q2 - 0.8/g.Nsnp;	
	float rho; 
	int iter=0; 

	float ll_det=1; 
	float prevll=0 ;
	MatrixXdr weight_mu = mu_k.cwiseProduct(alpha); 
	MatrixXdr residual = pheno - geno_matrix.transpose()*weight_mu; 
...
//
//
	boost::math::normal my_dist(0, 1);
	double  a =  pdf(my_dist, 0); 
	cout<<a<<endl;
	MAX_ITER=200;  
	...
	*/
		...
		svi_step( residual,mu_k, alpha, sigma_k, sigma_q1, sigma_q2, sigma_e, pi_q1, pi_q2, miniB,t, ll,step_a, step_b,iter); 
		if(iter>0)
			ll_det = ll - prevll;
		prevll = ll; 
		iter++; 
	...
	for(int i=0; i<g.Nindv; i++)
	{
		MatrixXdr curInd = geno_matrix.block(0, i, g.Nsnp, 1); 
		double dist_mean = curInd.cwiseProduct(mu_k).sum(); 
		boost::math::normal  my_dist(dist_mean, sqrt(sigma_e)); 
		double prob = pdf(my_dist, pheno(i,0)); 
		post *= prob;
	}
	for(int k=0; k<g.Nsnp;k++)
	{
		boost::math::normal  my_dist(0, sqrt(sigma_k(k,0)*sigma_e));
		double prob = pdf(my_dist, mu_k(k,0));
		post *= prob; 
		post *= alpha(k,0);  
	}
